...
※ブランチ OpenStreetMap になっています。  
※Google Map版と違いAPIキーは必要なく、そのままビルド実行で地図が表示されます。


![](Screenshot_osm01.png)

このような感じで Open Street Map による表示が行えます。
マーカー等は Google Map版と違います。
ここでは標準マーカーがドローンの位置を示します。赤マーカー（人型マーカー？）は操縦者もしくは離陸地点の位置を示します。
Google Map版 では吹き出しで情報が表示されていましたが、Open Street Map版ではマーカーをタップすると情報がToast表示されます。
またドローンが飛行した場所を示す赤い線は Google Map版と同じくマップ上に表示されます。

※赤（人型）マーカーはオリジナルのプロジェクトに組み込まれていたので、そのまま使用させて頂きました。  
